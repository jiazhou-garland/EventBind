...
import pandas as pd
from Dataloader.Imagenet.dataset import Nimagenet1k
    ...
    for step, (events, images, labels, real_num_frame) in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if cfg['MODEL']['TextEncoder']['init_ctx'] and cfg['MODEL']['TextEncoder']['leranable_ctx']:
                ...
                train_loss = train_loss + mse_loss
...
if __name__ == '__main__':
    # ---------------------------------------------------init----------------------------------------------------------------
    torch.cuda.empty_cache()
    cfg = read_yaml('./Configs/N-Imagenet.yaml')
    ...
    if cfg['Dataset']['Imagenet_dict_path'] != '':
        ...
        # print(classnames_num)
    # -----------------------------------------------dataset-----------------------------------------------------------------
    train_dataset = Nimagenet1k(cfg['Dataset']['Train']['Path'], cfg['Dataset']['Classnames'],
    ...
    train_loader = DataLoader(train_dataset, batch_size=cfg['Dataset']['Train']['Batch_size'],
                              ...
                              prefetch_factor=16, pin_memory=False)

    val_dataset = Nimagenet1k(cfg['Dataset']['Val']['Path'], cfg['Dataset']['Classnames'],
                              pad_frame_255=cfg['Dataset']['pad_frame_255'],
                              num_events=cfg['Dataset']['num_events'],
                              median_length=cfg['Dataset']['median_length'],
                              resize_width=cfg['Dataset']['resize_width'],
    ...
    best_acc1_im, best_acc5_im, best_acc1_ev, best_acc5_ev = 0,0,0,0
